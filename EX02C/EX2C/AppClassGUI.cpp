...
void Application::DrawGUI(void)
{
	...
	//About
	{
		...
		ImGui::Begin(sAbout.c_str(), (bool*)0, window_flags);
		{
			...
			vector3 v3Center = m_pObject1->GetCenterLocal();
			vector3 v3Min = m_pObject1->GetMinLocal();
			vector3 v3Max = m_pObject1->GetMaxLocal();
			vector3 v3Halfwidth = m_pObject1->GetHalfWidth();
			if (m_bColliding)
			{
				ImGui::TextColored(ImColor(255,255,255), "Colliding\n");
			}
			else
			{
				ImGui::TextColored(ImColor( 0, 255, 0), "Not Colliding\n");
			}
			float fGrade = 0;
			
			if (!IsClose(v3Center, vector3(3.0f, 5.0f, 0.0f)))
				ImGui::TextColored(v4Color, "Wrong Center!: %.3f, %.3f, %.3f\n", v3Center.x, v3Center.y, v3Center.z);
			else
				fGrade += 10.0f;

			if (!IsClose(v3Min, vector3(1.0f, 1.0f, -1.0f)))
			{
				ImGui::TextColored(v4Color, "Wrong Min!: %.3f, %.3f, %.3f\n", v3Min.x, v3Min.y, v3Min.z);
			}
			else
				fGrade += 15.0f;
			if (!IsClose(v3Max, vector3(5.0f, 9.0f, 1.0f)))
			{
				ImGui::TextColored(v4Color, "Wrong Max!: %.3f, %.3f, %.3f\n", v3Max.x, v3Max.y, v3Max.z);
			}
			else
				fGrade += 15.0f;
			if (!IsClose(v3Halfwidth, vector3(2.0f, 4.0f, 1.0f)))
			{
				ImGui::TextColored(v4Color, "Wrong Halfwidth!: %.3f, %.3f, %.3f\n", v3Halfwidth.x, v3Halfwidth.y, v3Halfwidth.z);
			}
			else
				fGrade += 10.0f;

			v4Color = ImColor(255, 255, 0);
			ImGui::TextColored(v4Color, "Grade: %.3f \n", fGrade);
			ImGui::TextColored(v4Color, "Add 20pts if ARBB is displaying correctly\n");
			ImGui::TextColored(v4Color, "Add 20pts if boxes are marking the right collision\n");
			ImGui::TextColored(v4Color, "Add 10pts if arrow is orienting correctly\n");
		}
		ImGui::End();
	}
